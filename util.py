...
def recursive_load_yaml(file: str) -> dict:
    ...
    if ("relative_path_list" in base_yaml) and (len(base_yaml["relative_path_list"]) > 0):
        repoRootDir = get_repo_root()
        for relative_path in base_yaml["relative_path_list"]:
            for i in range(len(base_yaml[relative_path])):
                base_yaml[relative_path][i] = os.path.join(repoRootDir, base_yaml[relative_path][i])
...
def dump_run_summary(path: str, taskResults: list):
    ...
    for result in taskResults:
        runSummaryFile.write(f"{result['time']}, {result['retCode']}, {'SUCCESS' if result['retCode'] == 0 else 'ERROR'}, {result['outputDir']}\n")
